	...
	while (true) {
		...
		case 1:
			...
			if (streaming::has_model_loaded(joaat("prop_phone_ing")) &&
				...
				streaming::has_anim_dict_loaded("amb@world_human_stand_mobile@female@standing@call@base") &&
				streaming::has_anim_dict_loaded("amb@world_human_stand_mobile@female@standing@call@exit") &&
				streaming::has_anim_dict_loaded("amb@world_human_smoking@female@enter") &&
				streaming::has_anim_dict_loaded("amb@world_human_smoking@female@idle_a")) {
				iLocal_37 = 2;
			}
			break;
	...
	case 0:
		if (entity::does_entity_exist(iLocal_38) && !entity::is_entity_dead(iLocal_38, 0)) {
			if (entity::is_entity_at_coord(iLocal_38, vLocal_57, 1f, 1f, 1f, 0, 1, 0)) {
				...
				ai::task_play_anim(0, "amb@world_human_stand_mobile@female@standing@call@enter", "enter", 4f, -4f, -1,
								   0, 0, 0, 0, 0);
				ai::task_play_anim(0, "amb@world_human_stand_mobile@female@standing@call@base", "base", 4f, -4f, -1, 1,
								   0, 0, 0, 0);
				ai::close_sequence_task(iVar6);
				ai::task_perform_sequence(iLocal_38, iVar6);
				ai::clear_sequence_task(&iVar6);
				iLocal_40 = 1;
			}
		}
			...
			if (ai::get_sequence_progress(iLocal_38) == 1) {
				if (!entity::does_entity_exist(iLocal_39)) {
					if (entity::is_entity_playing_anim(iLocal_38,
													   ...
													   "enter", 3)) {
						fVar7 =
							entity::get_entity_anim_current_time(iLocal_38,
																 "amb@world_human_stand_mobile@female@standing@call@"
																 "enter",
																 "enter");
						if (fVar7 >= 0.157f) {
							iLocal_39 = object::create_object(joaat("prop_phone_ing"), 1f, 1f, 1f, 1, 1, 0);
							entity::attach_entity_to_entity(iLocal_39, iLocal_38,
															ped::get_ped_bone_index(iLocal_38, 28422), 0f, 0f, 0f, 0f,
															0f, 0f, 1, 1, 0, 0, 2, 1);
						}
					}
				}
	...
	case 2:
		if (!entity::is_entity_dead(iLocal_38, 0)) {
			if (!func_3()) {
				ai::clear_sequence_task(&iVar6);
				ai::open_sequence_task(&iVar6);
				ai::task_play_anim(0, "amb@world_human_stand_mobile@female@standing@call@exit", "exit", 4f, -4f, -1, 0,
								   0, 0, 0, 0);
		...
		if (iVar4 == joaat("ig_tonya")) {
			...
			if (!func_23(vLocal_57)) {
				if (ai::get_script_task_status(iLocal_38, 713668775) != 1) {
					ai::task_follow_nav_mesh_to_coord(iLocal_38, vLocal_57, 1f, -1, 0.25f, 0, fLocal_60);
					ped::set_ped_keep_task(iLocal_38, 1);
				}
				return true;
			}
...
void func_24() {
	...
	if (entity::does_entity_exist(iLocal_38) && !entity::is_entity_dead(iLocal_38, 0)) {
		...
		if (func_23(vLocal_57)) {
			...
			if (!entity::is_entity_dead(player::player_ped_id(), 0)) {
				if (entity::is_entity_in_angled_area(player::player_ped_id(), 407.4285f, -1626.572f, 27.29278f,
