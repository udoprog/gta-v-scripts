...
void func_1() {
	switch (iLocal_21) {
	case 0:
		if (gameplay::get_game_timer() - iLocal_187 > 5000 &&
			entity::has_entity_been_damaged_by_entity(iLocal_188, player::player_ped_id(), 1)) {
			if (func_3(&uLocal_22, "NIGE3AU", "NIGEL3_REACT", 7, 0, 0, 0)) {
				entity::clear_entity_last_damage_entity(iLocal_188);
				iLocal_21 = 1;
			}
		}
		else if (gameplay::get_game_timer() - iLocal_187 > 12000 &&
				 system::vdist2(entity::get_entity_coords(player::player_ped_id(), 1),
								entity::get_entity_coords(iLocal_188, 1)) <= 900f) {
			if (func_3(&uLocal_22, "NIGE3AU", "NIGEL3_REACT", 7, 0, 0, 0)) {
				iLocal_21 = 1;
			}
		}
...
int func_17() {
	...
	if (system::vdist2(entity::get_entity_coords(player::player_ped_id(), 1),
					   entity::get_entity_coords(iLocal_188, 1)) >= fVar0 * fVar0 &&
		!entity::is_entity_on_screen(iLocal_188)) {
		return 0;
	}
	return 1;
}
